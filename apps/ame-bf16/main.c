...
void test_float_to_bf16(void) {
  ...
  /* Test cases: input float, expected bf16 value (known correct implementation
   * results) */
  struct {
    float f;
    ...
    const char *desc;
      ...
      {100.0f, 0x42C8, "100.0"},
      {-100.0f, 0xC2C8, "-100.0"},
      {0.0001f, 0x38D2, "0.0001 (round up)"},
      {-0.0001f, 0xB8D2, "-0.0001 (round up)"},
      ...
      {pos_inf, 0x7F80, "+inf"},
      {neg_inf, 0xFF80, "-inf"},
      {nan_val, 0x7FC0, "NaN"}, /* As long as output is NaN, mantissa doesn't
      ...
      {0.0078125f * (1 + 0x1.0p-8), 0x3C00, "2^-7 round up"},
  ...
  for (int i = 0; i < num_tests; i++) {
    float f = tests[i].f;
    uint16_t expected = tests[i].expected_bf16;
    const char *desc = tests[i].desc;
